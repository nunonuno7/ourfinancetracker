...
def transaction_bulk_duplicate(request):
    """
    Duplica transações selecionadas para um ou mais períodos de destino.
    """
    try:
        data = json.loads(request.body)
        transaction_ids = data.get("transaction_ids", [])
        ...
        if not transaction_ids:
            return JsonResponse({"success": False, "error": "Nenhuma transação selecionada"}, status=400)
        ...
        if not transactions.exists():
            return JsonResponse({"success": False, "error": "Nenhuma transação válida encontrada"})

        created_count = 0
        
        with db_transaction.atomic():
            # Se target_periods estiver vazio, duplicar no período atual (comportamento aproximado do original)
            if not target_periods:
                ...
                for tx in transactions:
                    new_tx = Transaction.objects.create(
                        ...
                        amount=tx.amount,
                        date=today,
                        notes=tx.notes or f"Duplicate of transaction from {tx.date}",
                        is_estimated=False,
                        period=current_period,
                        account=tx.account,
                    ...
                    if tx.tags.exists():
                        new_tx.tags.set(tx.tags.all())
                    created_count += 1
            else:
                # Duplicar para cada período selecionado
                for period_id in target_periods:
                    try:
                        period = DatePeriod.objects.get(id=period_id)
                        for tx in transactions:
                            # Tentar manter o dia, mas ajustar para o fim do mês se necessário
                            new_day = min(tx.date.day, monthrange(period.year, period.month)[1])
                            new_date = date(period.year, period.month, new_day)
        ...
        return JsonResponse({
            ...
            "message": f"{created_count} transações duplicadas com sucesso."
        })
    except Exception as e:
        logger.exception("Erro em bulk_duplicate")
        return JsonResponse({"success": False, "error": str(e)}, status=500)
